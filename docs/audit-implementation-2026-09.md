...
5. The discovery trigger accepts `Production` and Vercel’s project-qualified `Production – <project>` environment labels. The published release SHA remains the final guard. Confirm `release.json` matches the published frontend SHA and the discovery job runs. If the integration emits no event, manually run `npm run indexnow -- --submit` from frontend after checking the published release.
