...
export default function SupportersApp() {
  ...
  const shareSupport = async () => {
    const share = { title: 'World Mushroom Foraging', text: 'A free field guide and community map for mushrooms and herbs.', url: 'https://worldmushroomforaging.org/' }
    try {
      if (navigator.share) await navigator.share(share)
      else { await navigator.clipboard.writeText(share.url); setShareState('Link copied') }
    } catch (shareError) {
      if (shareError?.name !== 'AbortError') setShareState('Share link: worldmushroomforaging.org')
    }
  }
  return <div className={`supporters-shell ${herbs ? 'herbal-shell herbal-shell--forest' : 'mycelial-theme'}`}>
    ...
    <main className="supporters-main">
      <section className="supporters-intro" aria-labelledby="supporter-title">
        <div className="supporters-story"><p className="supporter-eyebrow">Optional support</p><h1 id="supporter-title">Field tools stay<br /><em>free for everyone.</em></h1><p className="supporter-lede">Support is appreciated, never necessary or expected. Supporters receive a few small thank-yous while every guide, map, and field tool stays open.</p><div className="supporter-friend-scene"><BotanicalSpecimen collection={herbs ? 'herbs' : 'fungi'} /><span>we grow better together</span></div></div>
        <div className="supporter-card">
          <p className="supporter-eyebrow"><Heart size={14} /> {status?.active ? 'Thank you for helping us grow' : 'The supporter circle'}</p>
          {status?.active ? <div className="supporter-welcome"><strong>You’re part of the circle.</strong><span>Your contribution helps keep this field guide open to every curious forager.</span></div> : <><div className="supporter-price"><strong>$10</strong><span>USD / year</span></div><p className="supporter-price-note">Optional support. All field tools remain free.</p></>}
          ...
          {user && <div className={`supporter-profile-preview${status?.active ? ' supporter-gilded' : ''}`}><span className={`supporter-avatar${status?.active ? ' supporter-gilded' : ''}`} aria-hidden="true">{user.username.slice(0, 1).toUpperCase()}</span><div><strong>{user.username}</strong><small>{status?.active ? 'Project supporter' : 'Your profile'}</small></div>{status?.active && <Check size={18} />}</div>}
          ...
          {status?.active && <div className="supporter-impact" aria-label="Your supporter benefits"><div><strong>40</strong><span>pocket poems unlocked</span></div><div><strong>1 year</strong><span>of supporter perks</span></div></div>}
          {user && status?.active && !membership.isError && <label className="supporter-listing-choice"><input type="checkbox" checked={publicListing} disabled={preference.isPending} onChange={event => { setListingChoice({ userId, value: event.target.checked }); preference.mutate(event.target.checked) }} /><span>Show <strong>{user.username}</strong> on the public supporter list.<small>Optional. You can change this here anytime.</small></span></label>}
          ...
          {status?.active && <><button className="supporter-share" type="button" onClick={shareSupport}><Share2 size={15} /> Share the free field guide</button>{shareState && <p className="supporter-share-state" role="status">{shareState}</p>}</>}
